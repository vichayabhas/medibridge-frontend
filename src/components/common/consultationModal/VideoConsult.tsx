...
export default function VideoConsult({ handoffId, pharmacyName, onClose }: { handoffId: string; pharmacyName: string; onClose: () => void }) {
  ...
  if (roomUrl === undefined) {
    return (
      <div className="flex flex-col items-center justify-center h-full bg-slate-950 text-white gap-3">
        ...
        <p className="text-sm text-slate-400">กำลังโหลดข้อมูลวิดีโอ...</p>
      </div>
    );
  }
  ...
  if (!roomUrl) {
    return (
      <div className="flex flex-col items-center justify-center h-full bg-slate-950 text-white gap-4 px-6">
        ...
        <p className="text-center text-sm text-slate-300">เภสัชกรยังไม่ได้เริ่มวิดีโอ<br />กรุณารอสักครู่แล้วลองใหม่</p>
  ...
  return (
    <DailyProvider userName="Patient" dailyConfig={{}}>
      <DailyCallInner roomUrl={roomUrl} audioOnly={false} pharmacyName={pharmacyName} onClose={onClose} handoffId={handoffId} />
    </DailyProvider>
  );
}
